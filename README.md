...
- **Phase 1** (MVP wire-up) — engineering complete; the recording is the only remaining gate. The tracing adapter is wired through to the control plane; policy decisions are produced on the ingest path in observe mode; an audit-ready evidence export carries a content hash an external auditor can recompute byte-for-byte. The operator surface now walks the full demo on one screen — agents observed, policy decisions, and a per-agent evidence-bundle export with a Download JSON link. A starter policy pack mapped to the EU AI Act risk tiers and PDPA cross-border references ships with the repo, alongside a Thai-and-English narration runbook and three Make one-liners that bring up the full demo without any incantation typed on camera. Next: capture the design-partner recording, then run the first conversations.
